...
Harvester.prototype.resource = function(name, resourceSchema, options) {
  ...
  let resourceSchemaClone = _.clone(resourceSchema);
  let legacyResourceSchema = this._convertToLegacySchema(resourceSchemaClone);
  let processedLegacySchema = this._preprocessSchema(legacyResourceSchema);
  // Store a copy of the input.
  this._schema[name] = _.clone(processedLegacySchema);
  this.changeHandlers[name] = [];

  let mongooseSchema = _this.adapter.schema(name, processedLegacySchema, options);
...
Harvester.prototype.listen = function() {
  ...
  this.server = router.listen.apply(router, arguments);
...
Harvester.prototype.noIndex = function(name) {
  ...
  let collection = this.options.inflect ? inflect.pluralize(name) : name;
  let index = [this.options.namespace, collection].join('/');
...
function allowCrossDomain(cors) {
  ...
  let headers = cors.headers || ['Accept', 'Content-Type', 'Authorization', 'X-Requested-With'];
  let methods = cors.methods || ['GET', 'PUT', 'POST', 'PATCH', 'DELETE'];
  let origins = cors.origins || '*';
  let credentials = cors.credentials || true;
...
module.exports = module.exports = create;
...
module.exports.JSONAPI_Error = require('./jsonapi-error');
