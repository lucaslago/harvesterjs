...
var shouldLog =  (process.env.LOG_EVENTS_READER=="true") ||false;
...
module.exports = function (harvesterApp) {
    ...
    return function (oplogMongodbUri, skip, success) {
        ...
        function matchChangeChandlers(changeHandlersPerResource, ns) {
            ...
            return _.chain(changeHandlersPerResource)
                .filter(function (changeHandler, resource) {
                    ...
                    shouldLog && log.profile(regexProfileMessage);
        ...
        var debouncedCheckpointUpdate = debounce(checkpointUpdate,parseInt(_.get(harvesterApp, 'options.eventsReaderDebounceWait'), 10) || 10000,{leading: true});
            ...
            if (doc != null) {
                ...
                if (!matchCheckpoint) {
                    ...
                    return Promise.resolve()
                        .then(function () {
                            shouldLog && log.profile(profileMessage);
                            debouncedCheckpointUpdate(that,doc)
                                .catch(function (err) {
                                    that.exit(err);
                                });
                            that.reschedule(0);
