...
module.exports = function (harvesterApp) {
    ...
    return function (oplogMongodbUri, skip, success) {
        ...
        function processWithHandler(that, changeHandler, doc, dfd) {
            var op = doc.op;
            console.log('OHVH -<> PLOG with doc', doc);
            // Skip cluster-internal operations
            if (doc.fromMigrate && that) {
              console.log('--------------------Start working from here - henrique "fromMigrate" : true--------');
              return that.skip(dfd, doc)
            }

            if (op === "i" || op === "u" || op === "d") {

                var id = (op === "u") ? id = doc.o2._id : id = doc.o._id;
